...
def dodaj_lekcje():
    ...
    kalendarz[dzien].sort(key=lambda x: x[0])
    odswiez_liste()
...
def odswiez_liste():
    ...
    for dzien, lekcje in kalendarz.items():
        ...
        plan_listbox.insert(tk.END, "")

def usun_lekcje():
    try:
        indeks = int(plan_listbox.curselection()[0]) - 1
    except IndexError:
...
def aktualna_godzina():
    ...
    root.after(1000, aktualna_godzina)

def dzisiaj():
    teraz = datetime.now()
    dzien = teraz.strftime("%A")
    data = teraz.strftime("%d-%m-%Y")
    dzien_label.config(text=f"Dzień: {dzien} | Data: {data}")
    root.after(60000, dzisiaj)
